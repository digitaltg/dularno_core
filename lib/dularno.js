...
class Kernel {
    ...
    #process_controller_function(ctrl, router, module) {
        ...
        for (const method of methods) {
            ...
            // Handle static information from CONTROLLER
            if (Object.keys(args).length == 0) {
                args.path = ctrl.constructor.paths[method]?.path;
                args.middlewares = ctrl.constructor.paths[method]?.middlewares;
            }
    ...
    #loadProviders(module, providers) {
        ...
        for (const providerName of  Object.keys(this.container.services[module.name])) {
            ...
            if (typeof provider.asyncPostInit === "function") {
                const old = provider.asyncPostInit.bind(provider);
                ...
                provider.asyncPostInit = newFn.bind(provider);
